...
void ByteArrayMutator::CrossOver(ByteArray &data, const ByteArray &other) {
  if (data.size() >= max_len_) {
    ...
    std::uniform_int_distribution<std::mt19937::result_type> zero_or_one(0, 1);
    bool default_choice = zero_or_one(rng_) == 0;
    if (knobs_.GenerateBool(knob_cross_over_insert_or_overwrite, default_choice,
                            rng_())) {
